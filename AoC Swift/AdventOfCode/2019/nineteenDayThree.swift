...
let testInputTwoB = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7" //distance 135
...
//        wireTwisteTwoString = testInputB.components(separatedBy: ",")
    ...
    func findMinimumDistance(){
        print("Calculating the Manhattan Distance...")

        for i in duplictedWirePath {
            //print("Duplicated WirePath: \(i.x),\(i.y)")
            let tempDist = manhattanDistance(endPoint: i)
            if (tempDist < minDistance) {
                minDistance = tempDist
            }
        }
        print("Manhattan Distance: \(minDistance) in" , Date().timeIntervalSince(date))
