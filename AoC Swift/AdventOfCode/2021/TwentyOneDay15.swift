...
class TwentyOneDay15 {
    
    private var input = In2021D15().getInput()
    let grid = In2021D15().getInput()
        .split(separator: "\n")
        .enumerated()
    ...
    private func PartOne() -> String {
        ...
        let finalPosition = Point(
            x: self.input.split(separator: "\n")[0].count - 1,
            y: self.input.split(separator: "\n").count - 1
        )
    ...
    private func PartTwo() -> String {
        let width = self.input.split(separator: "\n")[0].count
        let height = self.input.split(separator: "\n").count
    ...
    private func Tests() {
        assert(PartOne() == " ", "PartOne KO")
        assert(PartTwo() == " ", "PartTwo KO")
    }
