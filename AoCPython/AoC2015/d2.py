class d2:
    ...
    def wrap_total_order_sqf(self):
        total = 0
        total_ribbon = 0
        for i, v in enumerate(self.list_measures):
            
            l,w,h = [int(m) for m in v.split('x')]
            bow_ribbon = l*w*h
            print("bow ribbon", bow_ribbon)
            order_measures = [int(m) for m in v.split('x')]
            order_measures.sort()
            spare = [l*w, w*h, h*l]
            
            spare.sort()
            # formula 2*l*w + 2*w*h + 2*h*l
            #print(int(l),int(w),int(h), spare[0])
            gift = (2*l*w) + (2*w*h) + (2*h*l) + spare[0]
            print(order_measures[:2])
            wrap_ribbon = sum(order_measures[:2]) + sum(order_measures[:2])
            print("wrap ribbon", wrap_ribbon)
            total += gift
            '''
            ...
            total_ribbon = total_ribbon + bow_ribbon + wrap_ribbon
